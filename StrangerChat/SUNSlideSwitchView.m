...
{
    //创建顶部可滑动的tab
    _topScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 64, [UIScreen mainScreen].bounds.size.width, kHeightOfTopScrollView)];
    ...
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, kHeightOfTopScrollView, [[UIScreen mainScreen] bounds].size.width, 1)];
    [line setBackgroundColor:[UIColor grayColor]];
    [line setAlpha:0.4];
    [self addSubview:line];
    
    //创建主滚动视图
    _rootScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, kHeightOfTopScrollView+1+64, self.bounds.size.width, self.bounds.size.height - kHeightOfTopScrollView)];
...
- (void)layoutSubviews
{
    ...
    if (_isBuildUI) {
        ...
        if (self.rigthSideButton.bounds.size.width > 0) {
            ...
            _topScrollView.frame = CGRectMake(0, 64,
...
- (void)createNameButtons
{
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        CGSize textSize = [vc.title boundingRectWithSize:CGSizeMake([[UIScreen mainScreen] bounds].size.width, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:16]} context:nil].size;
...
//                                   lineBreakMode:NSLineBreakByTruncatingTail];
    ...
    //设置顶部滚动视图的内容总尺寸
    _topScrollView.contentSize = CGSizeMake(topScrollViewContentWidth+1000, kHeightOfTopScrollView);
}

- (void)createNameButtons_new
{
    ...
    if (_viewArray.count > 0) {
        if (_viewArray.count > 5) {
            width = 60;
        }else{
            width = [[UIScreen mainScreen] bounds].size.width/_viewArray.count;
        }
