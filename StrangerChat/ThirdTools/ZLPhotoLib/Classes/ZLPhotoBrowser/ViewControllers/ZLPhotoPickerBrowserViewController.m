...
@property (weak,nonatomic) UIButton         *likeBtn;// dahai
 ...
 *  点赞是否已经点击
 */
@property (assign,nonatomic) BOOL isLikeBtnSelected;
...
- (UIButton *)deleleBtn{
    ...
    likeBtn.frame = CGRectMake(CGRectGetMaxX([[UIScreen mainScreen] bounds])-90, CGRectGetMinY([[UIScreen mainScreen] bounds])+25, 20, 20);
...
/**
 *  点赞 a109：点赞用户ID，a108：photoId，a52：点赞用户昵称
 */
- (void)like{
    ...
    NSString *url = [NSString stringWithFormat:@"%@f_113_10_5.service?a109=%@&a108=%@&a52=%@&p1=%@&p2=%@&%@",kServerAddressTest2, _pointUserId, _photoId,_nickName,sessonId,userId,appinfoStr];
    ...
    [manger GET:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if ([codeNum integerValue] == 200) {
            // 保存点赞的记录id，为了取消点赞
            [[NSUserDefaults standardUserDefaults] setObject:[[infoDic objectForKey:@"body"] objectForKey:@"b34"] forKey:[NSString stringWithFormat:@"%@:%@",_pointUserId,_photoId]];
        }
...
- (void)disLike{
    ...
    manger.responseSerializer.acceptableContentTypes = [NSSet setWithObject:@"application/json"];
    NSString *appinfoStr = [NSGetTools getAppInfoString];
    NSString *url = [NSString stringWithFormat:@"%@f_113_11_5.service?a109=%@&a108=%@&p1=%@&p2=%@&%@",kServerAddressTest2, _pointUserId, _photoId,sessonId,userId,appinfoStr];
    url = [url stringByAddingPercentEscapesUsingEncoding: NSUTF8StringEncoding];
    [manger GET:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSData *datas = responseObject;
        NSString *result = [[NSString alloc] initWithData:datas encoding:NSUTF8StringEncoding];
