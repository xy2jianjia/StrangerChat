...
- (void)showLineWithButtonWidth:(CGFloat)width
{
    ...
    _line.backgroundColor = [UIColor redColor];// 滑动横线的颜色
...
- (NSArray *)getButtonsWidthWithTitles:(NSArray *)titles;
{
    ...
    {
        //CGSize size = [title sizeWithFont:[UIFont systemFontOfSize:[UIFont systemFontSize]]];// 过时了
...
- (void)setCurrentItemIndex:(NSInteger)currentItemIndex
{
    ...
    UIButton *button1 = _items[buttonIndex];
    

    
    // 代理传值
    //if (self.delegate != nil && [self.delegate respondsToSelector:@selector(sendValue:)]) {
        //[self.delegate sendValue:currentItemIndex];
           // NSLog(@"+=+====%ld",currentItemIndex);
    //}
    ...
    if (currentItemIndex != buttonIndex) {
        [_items[currentItemIndex] setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
       button.titleLabel.font = [UIFont systemFontOfSize:23.f];
        
        
        [_items[buttonIndex] setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        button1.titleLabel.font = [UIFont systemFontOfSize:18.f];
        
    }
    buttonIndex = currentItemIndex;
